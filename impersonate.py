...
model_path = 'model'
image_path1 = 'test_data/Helio_Castroneves/Helio_Castroneves_0001.png'
image_path2 = 'test_data/Zhang_Ziyi/Zhang_Ziyi_0003.png'
...
object_img = ref_img.copy()
cv2.imwrite('./impersonate_result_0.png', object_img)
...
with tf.Graph().as_default():
    with tf.Session() as sess:
        ...
        for t in range(T):
            ...
            # cv2.imwrite('process/impersonate_'+str(t)+'.png', object_img+z_hat)
            ...
            L_post = dis_post if (dist_post < 1.3) else float('inf')
            ...
            if t == 99 or t == 999 or t == 1999:
                cv2.imwrite('./impersonate_result_' + str(t+1) + '.png', object_img)
            MSE.append(mse(origin_img, object_img, image_size))

cv2.imwrite('./impersonate_result_10000.png', object_img)
...
plt.xlabel('queries')
plt.ylabel('MSE')
plt.title('Impersonate Attack')
plt.savefig('./Impersonate_Attack.png')